...
def render_quilt_image(colormap, seed=123, dpi=300, num=14, n_shapes=2000):
    """
...
axis_image1 = render_quilt_image(colormap=original_cmap, seed=2183, dpi=600)
# Render the second set
axis_image2 = render_quilt_image(colormap=opposite_cmap, seed=491, dpi=600)

# Combine via some pattern
pattern = lambda x, y: np.sin(x) + np.tan(y)
...
plt.axis('off')
plt.savefig(
    '/Users/henryschnieders/desktop/quilt_test_gooo00od.png',
    dpi=1200,
    bbox_inches='tight',
    pad_inches=0.0,
    transparent=True
