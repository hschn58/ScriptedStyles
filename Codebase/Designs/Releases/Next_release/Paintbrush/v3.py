    ...
    def paint(self, color, wsize, hsize, arr, top_left=(0,0)):
        ...
        for rr in range(row0, max_h):
            for cc in range(col0, max_w):
                ...
                if sum(arr[rr, cc, :]) !=0:
                    print(sum(arr[rr, cc,:]))
                    var = self.__brush(color, wsize, hsize, brush_w, brush_h)
                    for col_inf in range(4):
                        #arr[rr, cc, col_inf] = (arr[rr, cc, col_inf]*var[col_inf])
                        arr[rr, cc, col_inf] = abs(1-(arr[rr, cc, col_inf] + var[col_inf])/(1 + (var[col_inf]*arr[rr, cc, col_inf])))
                else:
